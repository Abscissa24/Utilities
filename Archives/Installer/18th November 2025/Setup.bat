...
:GUI_DETECTED
    ...
    echo.
    echo Recommended 'Y'
    echo Only use 'N' if you know what you are doing!
    echo. 
...
certutil -urlcache -split -f "%RemoteFileURL%" "%TempFile%" >nul 2>&1
...
COLOR 0E
echo WARNING: 🚧 MAINTENANCE 🚧 
echo. 
echo WARNING: Core functions are disabled during maintenance!
echo WARNING: Supplementary functions are available.
echo. 
pause 
goto MAIN_MENU1

:MAIN_MENU1 ::remove 1 after MAINTENANCE
cls
COLOR 0B
echo. 
echo  ██╗   ██╗████████╗██╗██╗     ██╗████████╗██╗███████╗███████╗
...
curl -L "%UTILITIES_URL%/Latest/Utilities-X.zip" -o "%TEMP_DIR%\Utilities.zip" >nul 2>&1
...
start "" "C:\Windows\Resources\Themes\aero.theme" >nul 2>&1
    ...
    powershell -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri 'https://github.com/Abscissa24/Utilities/raw/main/Assets/Themes/Release.zip' -OutFile '%TEMP%\Mawzes.zip' -UseBasicParsing" >nul 2>&1
...
:: Download the file
powershell -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri 'https://github.com/Abscissa24/Utilities/raw/refs/heads/main/Assets/Misc/Setup_Debug.bat' -OutFile '%AppData%\Utilities\Setup\Setup_Debug.bat' -UseBasicParsing" >nul 2>&1

:: Verify the file contains @echo off
findstr /m "^@echo off" "%AppData%\Utilities\Setup\Setup_Debug.bat" >nul 2>&1
if errorlevel 1 (
    cls
    echo ERROR: Schema Corrupted
    echo.
