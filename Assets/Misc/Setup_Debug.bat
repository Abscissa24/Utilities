...
:: Prompt for GUI switch and check if automatic update is possible
cls
...
if exist "%AppData%\Utilities\Setup\auto_update.conf" goto AUTO_UPDATE_BETA
if not exist "%AppData%\Utilities\Setup\Setup.bat" goto PRE_CONFIG
...
:: Step 1: Get the local version from the second line
set "LocalFile=%AppData%\Utilities\Setup\Setup.bat"
set "LineCount=0"
for /f "delims=" %%A in (%LocalFile%) do (
    set /a LineCount+=1
    if !LineCount! equ 2 set "LocalVersion=%%A"
)

:: Step 2: Download the remote file (temporarily)
set "RemoteFileURL=https://github.com/Abscissa24/Utilities/releases/download/Rolling-Release/Setup.bat"
set "TempFile=%TEMP%\RemoteSetup.bat"

:: Use certutil to download (Windows built-in)
certutil -urlcache -split -f "%RemoteFileURL%" "%TempFile%" 
...
:: Step 3: Get the remote version from second line
set "LineCount=0"
for /f "delims=" %%A in (%TempFile%) do (
    set /a LineCount+=1
    if !LineCount! equ 2 set "RemoteVersion=%%A"
)
:: Step 4: Compare versions (assuming YYYYMMDD format so string comparison is valid)
...
if not exist "%AppData%\Utilities\Setup\Setup.bat" (
    ...
    powershell -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri 'https://github.com/Abscissa24/Utilities/releases/download/Rolling-Release/Setup.bat' -OutFile '%AppData%\Utilities\Setup\Setup.bat' -UseBasicParsing" 
)
...
if not exist "%AppData%\Utilities\Shell\support.ico" (
    ...
    powershell -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri 'https://github.com/Abscissa24/Utilities/raw/refs/heads/main/Assets/support.ico' -OutFile '%AppData%\Utilities\Shell\support.ico' -UseBasicParsing" 
)
...
:MAIN_MENU
cls
title Utilities 10.102025 - Made by Armiel :)
COLOR 0B
echo. 
echo  ██╗   ██╗████████╗██╗██╗     ██╗████████╗██╗███████╗███████╗
echo  ██║   ██║╚══██╔══╝██║██║     ██║╚══██╔══╝██║██╔════╝██╔════╝
...
if not exist "C:\Utilities\Scripts" goto MAIN_MENU_OPTIONS
if exist "C:\Utilities\Scripts" goto MAIN_MENU_OPTIONS_ALT
::Main menu
...
if /i "%choice%"=="dev_enable_auto_update" goto AUTOUPDATE_ON
if /i "%choice%"=="dev_disable_auto_update" goto AUTOUPDATE_OFF
if /i "%choice%"=="dev_enable_logs" goto UPGRADE_WITH_LOGS
if /i "%choice%"=="dev_disablee_logs" goto UPGRADE
...
cls
COLOR 0E
mkdir "%AppData%\Utilities\Setup" 2>nul && echo AUTO_UPDATE_ENEABLED [EXPERIMENTAL] > "%AppData%\Utilities\Setup\auto_update.conf"
timeout /t 2 /nobreak 
goto AUTO_UPDATE_BETA

:AUTOUPDATE_OFF
COLOR 0B
del "%AppData%\Utilities\Setup\auto_update.conf"
cls
echo AUTO_UPDATE_DISABLED [STABLE]
timeout /t 2 /nobreak 
goto MAIN_MENU
...
echo. 
takeown /f "%AppData%\Utilities" /r /d y 
icacls "%AppData%\Utilities" /grant "%username%":F /t /q 

rd /s /q "%AppData%\Utilities" 
...
:: Download the file
powershell -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri 'https://github.com/Abscissa24/Utilities/raw/refs/heads/main/Archives/Installer/Setup.bat' -OutFile '%AppData%\Utilities\Setup\Setup.bat' -UseBasicParsing" 

:: Verify the file contains @echo off
findstr /m "^@echo off" "%AppData%\Utilities\Setup\Setup.bat" 
if errorlevel 1 (
    cls
...
if not exist "C:\Utilities\Data\Shortcuts" (
    mkdir "C:\Utilities\Data\Shortcuts" 
)
...
if "%choice%"=="1" goto VERSION_X_OCTOBER
if "%choice%"=="2" goto VERSION_X_SEPTEMBER
if "%choice%"=="3" goto VERSION_X_AUGUST
if "%choice%"=="4" goto VERSION_X_JUNE
if "%choice%"=="5" goto VERSION_X_MAY
if "%choice%"=="6" goto VERSION_X_APRIL
if "%choice%"=="7" goto VERSION_X_MARCH
...
curl -L "%UTILITIES_URL%/Latest/Utilities-X.zip" -o "%TEMP_DIR%\Utilities.zip" 
powershell -Command "Expand-Archive -Path '%TEMP_DIR%\Utilities.zip' -DestinationPath '%INSTALL_DIR%' -Force" 
...
setx /M PATH "!PATH!;%INSTALL_DIR%\Data\Shortcuts" 
setx /M PATH "!PATH!;%AppData%\Utilities\Setup" 
...
powershell -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri '%UTILITIES_URL%/Latest/Utilities-X.zip' -OutFile '%TEMP_DIR%\Utilities.zip' -UseBasicParsing" 
powershell -Command "Expand-Archive -Path '%TEMP_DIR%\Utilities.zip' -DestinationPath '%INSTALL_DIR%' -Force" 
...
powershell -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri 'https://github.com/Universal-Debloater-Alliance/universal-android-debloater-next-generation/releases/download/v1.1.2/uad-ng-windows.exe' -OutFile 'C:\Utilities\Data\ADB\uad-ng-windows.exe' -UseBasicParsing" 

powershell -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri 'https://github.com/Genymobile/scrcpy/releases/download/v3.3.2/scrcpy-win64-v3.3.2.zip -UseBasicParsing" 
powershell -Command "Expand-Archive -Path '%TEMP_DIR%\scrcpy-win64-v3.3.2.zip' -DestinationPath '%INSTALL_DIR%\Data\Record' -Force" 

:: Install additional experimental components
powershell -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri 'https://github.com/koush/adb.clockworkmod.com/releases/latest/download/UniversalAdbDriverSetup.msi' -OutFile '%TEMP_DIR%\UniversalAdbDriverSetup.msi' -UseBasicParsing" 
msiexec /i "%TEMP_DIR%\UniversalAdbDriverSetup.msi" /qn /norestart /passive 

setx /M PATH "!PATH!;%INSTALL_DIR%\Data\ADB" 
setx /M PATH "!PATH!;%AppData%\Utilities\Setup" 
setx /M PATH "!PATH!;%INSTALL_DIR%\Data\Shortcuts" 

powershell -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri '%UTILITIES_URL%/Latest/Experimental-X.zip' -OutFile '%TEMP_DIR%\Utilities.zip' -UseBasicParsing" 
powershell -Command "Expand-Archive -Path '%TEMP_DIR%\Utilities.zip' -DestinationPath '%INSTALL_DIR%' -Force" 
regedit /s "%INSTALL_DIR%\Registry\Experimental.reg" 
...
powershell -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri 'https://github.com/Abscissa24/Utilities/raw/main/Archives/Version-%version%/Utilities-%version%.zip' -OutFile '%TEMP_DIR%\Utilities.zip' -UseBasicParsing" 
powershell -Command "Expand-Archive -Path '%TEMP_DIR%\Utilities.zip' -DestinationPath '%INSTALL_DIR%' -Force" 

rmdir /s /q "%INSTALL_DIR%\Data\Shortcuts" 

powershell -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri 'https://github.com/Abscissa24/Utilities/raw/main/Archives/Version-%version%/Experimental-%version:~-1%.zip' -OutFile '%TEMP_DIR%\Utilities.zip' -UseBasicParsing" 
powershell -Command "Expand-Archive -Path '%TEMP_DIR%\Utilities.zip' -DestinationPath '%INSTALL_DIR%' -Force" 
...
if exist "C:\Utilities\Info\README.pdf" (
    start "" "C:\Utilities\Info\README.pdf" 
) else if exist "C:\Utilities\Info\README.txt" (
    start "" "C:\Utilities\Info\README.txt" 
) else if exist "C:\Utilities\Info\Info.pdf" (
    start "" "C:\Utilities\Info\Info.pdf" 
...
cls
echo Performing shell integration
powershell -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri '%UTILITIES_URL%/Archives/Version-X/%version%/Utilities-X.zip' -OutFile '%TEMP_DIR%\Utilities.zip' -UseBasicParsing" 
powershell -Command "Expand-Archive -Path '%TEMP_DIR%\Utilities.zip' -DestinationPath '%INSTALL_DIR%' -Force" 

powershell -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri '%UTILITIES_URL%/Archives/Version-X/%version%/Experimental-X.zip' -OutFile '%TEMP_DIR%\Experimental.zip' -UseBasicParsing" 
powershell -Command "Expand-Archive -Path '%TEMP_DIR%\Experimental.zip' -DestinationPath '%INSTALL_DIR%' -Force" 

powershell -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri 'https://github.com/Universal-Debloater-Alliance/universal-android-debloater-next-generation/releases/download/v1.1.2/uad-ng-windows.exe' -OutFile '%INSTALL_DIR%\Data\Misc\uad-ng-windows.exe' -UseBasicParsing" 

powershell -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri 'https://github.com/Genymobile/scrcpy/releases/download/v3.3.2/scrcpy-win64-v3.3.2.zip' -OutFile '%TEMP_DIR%\scrcpy-win64-v3.3.2.zip' -UseBasicParsing" 
powershell -Command "Expand-Archive -Path '%TEMP_DIR%\scrcpy-win64-v3.3.2.zip' -DestinationPath '%INSTALL_DIR%\Data\Record' -Force" 
...
robocopy "%INSTALL_DIR%\Shell" "%SHELL_DIR%" shell.nss /COPYALL /R:3 /W:5 
robocopy "%INSTALL_DIR%\Shell" "%SHELL_DIR%" support.ico /COPYALL /R:3 /W:5 
regedit /s "%INSTALL_DIR%\Registry\Utilities.reg"
regedit /s "%INSTALL_DIR%\Registry\Experimental.reg" 
...
setx /M PATH "!PATH!;%INSTALL_DIR%\Data\ADB" 
setx /M PATH "!PATH!;%AppData%\Utilities\Setup" 
setx /M PATH "!PATH!;%AppData%\Utilities\Shortcuts" 

timeout /t 1 /nobreak 

del /f /q "%USERPROFILE%\Desktop\Wise Program Uninstaller.lnk" 
...
reg delete "HKEY_CLASSES_ROOT\Directory\Background\shell\Z002AAC\shell\Z007ACD" /f 
reg delete "HKEY_CLASSES_ROOT\Directory\background\shell\Z002AAC\shell\Z007ACE" /f 

takeown /f "%INSTALL_DIR%\Scripts\Experimental" /r /d y 
icacls "%INSTALL_DIR%\Scripts\Experimental" /grant %username%:F 
rmdir /s /q "%INSTALL_DIR%\Scripts\Experimental" 
rmdir /s /q "%INSTALL_DIR%\Data\Record" 
del /f /q "%INSTALL_DIR%\Info\Experimental.pdf" 
...
for %%F in (clean.lnk lag.lnk insomnia.lnk speed.lnk tidy.lnk dmu.lnk) do (
    if exist "C:\Utilities\Data\Shortcuts\%%F" (
        copy /y "C:\Utilities\Data\Shortcuts\%%F" "%USERPROFILE%\Desktop\" 
    )
)
...
powershell -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri 'https://github.com/Abscissa24/Utilities/releases/download/GUI/Utilities.exe' -OutFile '%USERPROFILE%\Desktop\Utilities.exe' -UseBasicParsing" 
timeout /t 1 /nobreak 
...
echo Setup Version: 10.10102025
...
echo. 
echo Build Date: 10th October 2025
...
echo 2) Theme Override
echo 3) Remove all configurations
echo.
...
del IconCache.db /a 
del /q /f /s "%localappdata%\Microsoft\Windows\Themes\*.*" && rmdir /s /q "%localappdata%\Microsoft\Windows\Themes\" 
cls
...
start "" "C:\Windows\Resources\Themes\aero.theme" 
    ...
    echo. 
    powershell -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri 'https://github.com/Abscissa24/Utilities/raw/main/Assets/Themes/Mawzes.zip' -OutFile '%TEMP%\Mawzes.zip' -UseBasicParsing" 
    powershell -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri 'https://github.com/Abscissa24/Utilities/raw/main/Assets/Themes/Release.zip' -OutFile '%TEMP%\Mawzes.zip' -UseBasicParsing" 
    
    powershell -Command "Expand-Archive -Path '%TEMP%\Mawzes.zip' -DestinationPath '%AppData%\Utilities\Themes' -Force" 
    powershell -Command "Expand-Archive -Path '%TEMP%\Release.zip' -DestinationPath '%AppData%\Utilities\Themes' -Force" 
    timeout /t 2 /nobreak 
    ...
    regsvr32 /s "%AppData%\Utilities\Themes\Release\ExplorerBlurMica.dll" 
    cls

    reg add "HKCU\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced" /v IconsOnly /t REG_DWORD /d 1 /f
    reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Shell Icons" /v 3 /t REG_SZ /d "%AppData%\Utilities\Themes\Mawzes\Icons\folder.ico,0" /f 
    reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Shell Icons" /v 4 /t REG_SZ /d "%AppData%\Utilities\Themes\Mawzes\Icons\folder.ico,0" /f 
    ...
    start "" "%AppData%\Utilities\Themes\Mawzes\Mawzes.theme" 
)
call :CLEANUP 
...
echo Syncing Theme Repository: Mawzes
    ...
    echo. 
    powershell -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri 'https://github.com/Abscissa24/Utilities/raw/main/Assets/Themes/Mawzes.zip' -OutFile '%TEMP%\Themes\Mawzes.zip' -UseBasicParsing" 
    powershell -Command "Expand-Archive -Path '%TEMP%\Mawzes.zip' -DestinationPath '%AppData%\Utilities\Themes' -Force" 
    timeout /t 2 /nobreak 
...
:MAWZES_UNINSTALL
if exist "C:\Utilities\Uninstall\unins000.exe" (
start "" "C:\Utilities\Uninstall\unins000.exe" 
)
winget uninstall --id=Nilesoft.Shell 
winget uninstall --id=StartIsBack.StartAllBack 
call :UNINSTALL_ALL
timeout /t 2 /nobreak 
call :REVERT
...
echo.
powershell -Command "Expand-Archive -Path '%USERPROFILE%\Downloads\Resources.zip' -DestinationPath '%USERPROFILE%\Downloads\Resources' -Force" 
cls
echo Installing
...
powershell -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri 'https://github.com/Abscissa24/Utilities/raw/refs/heads/main/Assets/Misc/authentication.ini' -OutFile '%PASS_FILE%' -UseBasicParsing" 
...
if exist "%TEMP%\Utilities-main" (
    ...
    powershell -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri 'https://github.com/Abscissa24/Utilities/releases/download/Rolling-Release/Setup.bat' -OutFile '%USERPROFILE%\Desktop\GitHub Development\Setup.bat' -UseBasicParsing" 
)
echo. 
...
if exist "%AppData%\Utilities\Themes\Release" goto SYNC_THEMES

:SYNC_THEMES
cls
echo Syncing Theme Repository
 powershell -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri 'https://github.com/Abscissa24/Utilities/raw/main/Assets/Themes/Release.zip' -OutFile '%TEMP%\Release.zip' -UseBasicParsing" 
    powershell -Command "Expand-Archive -Path '%TEMP%\Release.zip' -DestinationPath '%AppData%\Utilities\Themes' -Force" 
goto THEMES
...
if "%choice%"=="mawzes" goto MAWZES_THEME
...
cls
 powershell -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri 'https://github.com/Abscissa24/Utilities/raw/main/Assets/Themes/SamuraiCatto.zip' -OutFile '%TEMP%\SamuraiCatto.zip' -UseBasicParsing" 
    powershell -Command "Expand-Archive -Path '%TEMP%\SamuraiCatto.zip' -DestinationPath '%AppData%\Utilities\Themes' -Force" 
)
...
reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Shell Icons" /v 3 /t REG_SZ /d "%AppData%\Utilities\Themes\SamuraiCatto\Icons\folder.ico,0" /f 
reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Shell Icons" /v 4 /t REG_SZ /d "%AppData%\Utilities\Themes\SamuraiCatto\Icons\folder.ico,0" /f 
...
start "" "%AppData%\Utilities\Themes\SamuraiCatto\SamuraiCatto.theme" 
...
if not exist "%AppData%\Utilities\Themes\Pink" (
 powershell -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri 'https://github.com/Abscissa24/Utilities/raw/main/Assets/Themes/Pink.zip' -OutFile '%TEMP%\Pink.zip' -UseBasicParsing" 
    powershell -Command "Expand-Archive -Path '%TEMP%\Pink.zip' -DestinationPath '%AppData%\Utilities\Themes' -Force" 
)
...
reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Shell Icons" /v 3 /t REG_SZ /d "%AppData%\Utilities\Themes\Pink\Icons\folder.ico,0" /f 
reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Shell Icons" /v 4 /t REG_SZ /d "%AppData%\Utilities\Themes\Pink\Icons\folder.ico,0" /f 
...
start "" "%AppData%\Utilities\Themes\Pink\Pink.theme" 
...
if not exist "%AppData%\Utilities\Themes\Bubu" (
 powershell -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri 'https://github.com/Abscissa24/Utilities/raw/main/Assets/Themes/Bubu.zip' -OutFile '%TEMP%\Bubu.zip' -UseBasicParsing" 
    powershell -Command "Expand-Archive -Path '%TEMP%\Bubu.zip' -DestinationPath '%AppData%\Utilities\Themes' -Force" 
)
...
reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Shell Icons" /v 3 /t REG_SZ /d "%AppData%\Utilities\Themes\Bubu\Icons\folder.ico,0" /f 
reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Shell Icons" /v 4 /t REG_SZ /d "%AppData%\Utilities\Themes\Bubu\Icons\folder.ico,0" /f 
...
start "" "%AppData%\Utilities\Themes\Bubu\BubuDudu.theme" 
...
if not exist "%AppData%\Utilities\Themes\Abstract" (
 powershell -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri 'https://github.com/Abscissa24/Utilities/raw/main/Assets/Themes/Abstract.zip' -OutFile '%TEMP%\Abstract.zip' -UseBasicParsing" 
    powershell -Command "Expand-Archive -Path '%TEMP%\Abstract.zip' -DestinationPath '%AppData%\Utilities\Themes' -Force" 
)
...
reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Shell Icons" /v 3 /t REG_SZ /d "%AppData%\Utilities\Themes\Abstract\Icons\folder.ico,0" /f 
reg add "HKLM\SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Shell Icons" /v 4 /t REG_SZ /d "%AppData%\Utilities\Themes\Abstract\Icons\folder.ico,0" /f 
...
start "" "%AppData%\Utilities\Themes\Abstract\Abstract.theme" 
...
del "%TEMP%\AppInstaller.appxbundle" 
timeout /t 2 /nobreak >nul
...
echo.     
set /p choice=Repair Installation (R) or Update Activation (U): 

if /i "%choice%"=="R" goto REPAIR_OFFICE
if /i "%choice%"=="U" goto UPDATE_OFFICE_ACTIVATION
    
:REPAIR_OFFICE   
COLOR 0A
cls
echo Repairing Office Installation

powershell -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri 'https://github.com/Abscissa24/Utilities/raw/refs/heads/main/Assets/Office.zip' -OutFile '%SystemRoot%\Temp\Office.zip' -UseBasicParsing" 
powershell -Command "Expand-Archive -Path '%SystemRoot%\Temp\Office.zip' -DestinationPath '%Appdata%\Utilities\Office' -Force" 
...
cls
COLOR 0E
echo DEBUGGING_MODE_ENABLED
echo.                                                                         
takeown /f "%AppData%\Utilities" /r /d y 
icacls "%AppData%\Utilities" /grant "%username%":F /t /q 

:: Commenting out because its not really needed
:: rd /s /q "%AppData%\Utilities" 
...
:: Download the file
powershell -Command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; Invoke-WebRequest -Uri 'https://github.com/Abscissa24/Utilities/raw/refs/heads/main/Assets/Misc/Setup_Debug.bat' -OutFile '%AppData%\Utilities\Setup\Setup_Debug.bat' -UseBasicParsing" 

:: Verify the file contains @echo off
findstr /m "^@echo off" "%AppData%\Utilities\Setup\Setup_Debug.bat" 
if errorlevel 1 (
    cls
    echo ERROR: Schema Corrupted
    echo.
...
cls
call "%AppData%\Utilities\Setup\Setup_Debug.bat"
exit
...
start "C:\Windows\Utilities\Unins000.exe" 
)
...
if exist "%INSTALL_DIR%" (
    rmdir /s /q "%INSTALL_DIR%\Data\Misc" 
    rmdir /s /q "%INSTALL_DIR%\Registry" 
)
goto :EOF
